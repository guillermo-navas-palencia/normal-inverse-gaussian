...
double estimate_magnitude_bisection(
  const double x,
  const double alpha,
  const double beta,
  const double mu,
...
)
{
  ...
  // Bracket: 1e-6, 2 * delta
  double a = 1e-6;
  double b = 2.0 * delta + a;

  double x0, fc, fa;
  for (int k = 0; k < maxiter; k++)
  {
    x0 = (a + b) * 0.5;
    integrand(x0, beta, xmu, gamma2, delta2, fc);

    if ((fc == 0.0) | ((b - a) * 0.5 < tol))
      break;

    integrand(a, beta, xmu, gamma2, delta2, fa);
...
)
{
  ...
  // bool use_asymp2 = alpha2 + xmu2 < 2.0 * beta2;
...
)
{
  ...
  // double magnitude_b = estimate_magnitude_bisection(x, alpha, beta, mu, delta, gamma, C);
  // std::cout << magnitude << " " << magnitude_b << std::endl;

  // Estimate the truncation point N
  int N = truncation(delta, gamma, eps * std::min(magnitude, 1.0));

  // if (!std::isnan(magnitude)) {
  //   int Nm = truncation(delta, gamma, eps * std::min(magnitude, 1.0));
  //   N = std::max(N, Nm);
  // }
