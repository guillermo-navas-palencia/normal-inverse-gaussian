...
\end{align}
where $\omega = \sqrt{(x-\mu)^2 + \delta^2}$. Note that the remainder is bounded in magnitude by the first neglected term. Moreover, the convergence of the expansion improves for large $\alpha$ and $\alpha / \omega > 1$. Finally, for $x - \mu > 0$ we use the reflection formula \eqref{cdf_mirror}.
...
\subsection{Expansions: general case}\label{section_general_case}
...
\end{equation}
implying that we can safely use this approximation when the following inequality is satisfied (which should occur for reasonable parameters)
\begin{equation}
\delta \gamma e^{\delta\gamma} > \sqrt{\frac{27 \pi}{2}} \epsilon.
\end{equation}
Note that, from a computational perspective, the use of the upper bound \eqref{lambertW0_upper_bound} is beneficial to avoid overflow/underflow issues for large parameters.
...
\subsubsection{Integral representation in terms of elementary functions}
The common integral representations presented in Section \ref{properties_cdf} involve special functions in their integrands (e.g., $K_1(x)$ or $\erfc(x)$), which necessarily increases the computation cost of their evaluation. Hence, from a computational perspective, an integral representation in terms of elementary functions is preferred. In the following, we derive an integral representation using the sine transform of the modified Bessel function.
...
\end{equation}
and the reflection formula \eqref{cdf_mirror} is used for $x-\mu < 0$.
\end{proposition}
\begin{proof}
...
\begin{itemize}
\item Truncate integral and bound tail.
\item Estimate $N$ solving equation.
\item Complex integration. Saddle-point method for large $\delta$.
\end{itemize}

The sine integral can be written in complex form
...
In a similar manner, we can obtain an integral representation in terms of the exponential integral using the cosine transform of the modified Bessel function.
