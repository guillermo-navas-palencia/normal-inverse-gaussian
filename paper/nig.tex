...
\end{equation}
To obtain series expansion for $|x-\mu| \to 0$, we can proceed expanding $\Phi\left(\frac{x-\mu}{\sqrt{t}}\right)$, using the two well-known absolutely convergent series expansions of $\Phi(x)$ \cite[\S 2]{Lebedev1972}
...
and
\begin{equation*}
F(x; \alpha, 0, \mu, \delta) = \frac{1}{2} + \frac{\delta e^{\delta \gamma}}{\pi}\frac{(x-\mu)\alpha}{\omega}\sum_{k=0}^{N-1}T_k + R_N.
\end{equation*}
...
\begin{itemize}
\item Show bound for alternating sum
\item Approximation of the number of terms solving $T_N$ in terms of Lambert.
\item Tables with examples of bound accuracy.
\item Decision about the region of usability for each expansion.
\end{itemize}
