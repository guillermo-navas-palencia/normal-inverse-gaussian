...
\hline
    ...
    1/3 & 10 & 1/5 & 1/50 & 1.35 & 1494 & $2.1\cdot 10^{-16}$ & $2.2\cdot 10^{-16}$\\
...
TODO: Nevertheless, in practice, the main purpose of the estimation of $N$ using \eqref{N_expansion_xmu_b0_positive} is to decide whether the series expansion should selected as the method of computation given a certain parameter region. For a detailed discussion, we refer to Section \ref{subsection_implementation}.
...
\end{equation}
Using $\lim_{x \to 0}\, _2F_1\left(1, 1, \frac{3}{2}-k, z\right) = 1$, we have to solve for $N$
\begin{equation}
\frac{(\alpha\omega)^{2N}}{(2N)!} \Gamma(N - 1/2) = \epsilon
\end{equation}

\begin{align}
\frac{\Gamma(N - 1/2)}{(2N)!} &\sim \sqrt{2} \sqrt{\frac{2N + 1}{2N - 1}} \frac{(N - 1/2)^{N - 1/2}}{(2N + 1)^{2N + 1}} e^{N + 3/2}, \quad N \to \infty\\
&\le 2^{-2N - 1/2} \left(\frac{e}{N}\right)^N
\end{align}
\begin{equation}
(\alpha\omega)^{2N} 2^{-2N - 1/2} \left(\frac{e}{N}\right)^N = \epsilon, \quad N \approx - \frac{\log(\epsilon\sqrt{2})}{W\left(-\frac{\log(\epsilon\sqrt{2})}{e^{\alpha\omega/2}} \right)}
\end{equation}
