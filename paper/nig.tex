...
\end{equation}
Note that, from the implementation perspective, the use of the upper bound  \eqref{lambertW0_upper_bound} to estimate $N$ avoids overflow/underflow issues for large parameters due to direct use of logarithms.
...
\end{align*}
where we use the fact that $|\sin(\delta t)| \le 1$, and that a bound for the latter integral can be expressed in closed form using \cite[\S 3.461]{gradshteyn2007}
\begin{equation*}
...
To select $N$, we take the latter bound and equate to a prescribed accuracy $\epsilon$. The solution admits a closed-form solution in terms of $W_0(z)$. Thus, we solve
\begin{equation}
\frac{e^{-(x-\mu)\left(\sqrt{N^2 + \alpha^2} - \beta\right)}}{\sqrt{N^2 + \alpha^2} - \beta} = \frac{\epsilon}{C}
\end{equation}
obtaining
\begin{equation}
N = \frac{1}{x-\mu}\sqrt{(x-\mu)^2(\beta^2 - \alpha^2) + 2\beta(x-\mu) W_0\left(\frac{(x-\mu) C}{\epsilon}\right) + W_0\left(\frac{(x-\mu) C}{\epsilon}\right)^2}
\end{equation}
