...
Decision tree. Machine Learning approach \cite{Simpson2016}. Some details about ML model are out of the scope. The optimal regimes of applicability of the different series and asymptotic expansion could be derived using the information provided by the bounds on their remainders. To decide which method should be used in each regime, one could formulate an optimization problem with two possible objectives, minimization of terms (series complexity) and maximize robustness / reliability, or a blended approach (multi-objective optimization). 
...
Expansion $|x-\mu| \to 0$ in terms of incomplete gamma: Numerical instability prevents its implementation in double-precision arithmetic. 
