...
Now we are in position to compare both series and their corresponding domains of usability. A first important observation is that the alternating series \eqref{expansion_xmu_b0_alternating} does not converge when $|x - \mu| > \delta$, and the number of terms $N$ increases rapidly when $|x - \mu| \sim \delta$. In contrast, the series \eqref{expansion_xmu_b0_positive} is absolutely convergent, although $N$ also grows considerably when $\alpha\omega \to 0$. The convergence the latter can be reliably assessed using the fact that $|x-\mu|/\omega < 1$, then from \eqref{expansion_xmu_b0_positive_error_term} we have
\begin{equation*}
T_k = \left(\frac{(x-\mu)^2 \alpha}{\omega}\right)^k \frac{K_{k+1}(\alpha \omega)}{(2k +1)!!} < (|x-\mu|\alpha)^k \frac{K_{k+1}(\alpha \omega)}{(2k +1)!!},
\end{equation*}
and taking the asymptotic estimates of $K_{k+1}(\alpha\omega)$ for $\alpha \omega \to \infty$ and $\alpha \omega \to 0$ [References to Appendix]:
\begin{align*}
T_k &\sim \sqrt{\frac{\pi}{2\alpha \omega}} \frac{|x-\mu|^k \alpha^k e^{-\alpha \omega}}{(2k + 1)!!}, &\alpha\omega \to \infty\\
T_k &\sim \frac{\pi}{2 \alpha \omega} \frac{\Gamma(k + 1)}{\Gamma(k + 3/2)}, & \alpha\omega \to 0.
\end{align*}
From the previous asymptotic estimates of $T_k$, we observe that the series expansion \eqref{expansion_xmu_b0_positive} is slowly convergent for $\alpha\omega \to 0$. Conversely, only a few terms are required for $\alpha\omega \to \infty$ when using machine precision. It is worth noticing that performing a naive computation of the terms $T_k$ in double-precision arithmetic for large $N$ poses underflow and overflow problems since the numerator (denominator) rapidly goes to infinity (zero) as $N$ increases. In Section \ref{subsection_evaluation_bessel_expansions}, we discuss alternative summation methods to avoid cancellation issues and precision loss, and the usage of convergence acceleration methods for the case $\alpha\omega \to 0$.


\begin{itemize}
\item Tables with examples of bound accuracy.
\end{itemize}
