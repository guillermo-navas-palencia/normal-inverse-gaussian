...
\item Processor:
\item Compilers
\item Number of cases and range.
\end{itemize}

TODO: comment about not claiming the same behaviour in other hardware configurations and compilers (Windows, for instance).


\subsection{Numerical integration}\label{algorithmic_numerical_integration}

Double-exponential quadrature: \cite{Takahasi1973}.
...
Mention the numerical integration method implemented in other libraries where the NIG distribution is available. QUADPACK \cite{QUADPACK}. Implementation inspired by the Python library \texttt{tanh-sinh} \url{https://github.com/sigma-py/tanh-sinh}.
